...
# Requirements → Component Allocation (Phase 5 Refine Links)
...
### Path rule for System Composer

Never use `'..'` in paths passed to System Composer — use `fileparts` twice to
get the project root, then `addpath` before opening the model by name:

```matlab
rootDir = fileparts(fileparts(mfilename('fullpath')));
addpath(fullfile(rootDir, 'architecture'));
